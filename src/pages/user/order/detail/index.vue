...
const getOrderInfo = async () => {
  let result: OrderResponseData = await reqOrderInfo({id:route.query.orderId as string});
  if (result.code == 200) {
...
const cancel = async () => {
  try {
    //取消预约成功
    await reqCancelOrder({id:route.query.orderId as string});
    //再次获取订单详情的数据
    getOrderInfo();
  } catch (error) {
    ElMessage({
...
const openDialog = async () => {
  ...
  //获取支付需要使用二维码信息
  let result: QrCode = await reqQrcode({orderId:route.query.orderId as string});
  //更具服务器返回二维码信息生成二维码图片
  ...
  timer.value = setInterval(async () => {
    let result: PayReslt = await reqQueryPayState({orderId:route.query.orderId as string});
