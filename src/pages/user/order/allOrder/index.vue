...
const getOrderInfo = async (pager: number = 1) => {
  pageNo.value = pager;
  let result: UserOrderInfoResponseData = await reqUserOrderInfo({page: pageNo.value, limit: pageSize.value, patientId: patientId.value, orderStatus: orderStatus.value});
  if (result.code == 200) {
